...
const specsList=Array.isArray(item.brief)?item.brief:(item.brief?item.brief.split('\n'):[]);const specsHTML=specsList.length>0?specsList.map(s=>`<li>${s}</li>`).join(''):'<li>N/A</li>';const panelSliderHTML=(item.images||[]).map(src=>{const fullSrc=src.replace(/_thumb(\.[^.]+)$/,'$1');return`<img class="panel-full-img"loading="lazy"decoding="async"src="${fullSrc}"alt="${item.name}">`;}).join('');const shippingText=parsePrice(item.shipping)>0?`+${getFormattedPrice(parsePrice(item.shipping))}shipping`:'Free shipping';let badgeHtml='';if(item.essentialRank===1){badgeHtml=`<div class="essential-badge">ESSENTIAL<span class="panel-badge-rank">${item.essentialRank}</span></div>`;}else if(item.essentialRank>1){badgeHtml=`<div class="rank-badge">RANK<span class="panel-badge-rank">${item.essentialRank}</span></div>`;}
